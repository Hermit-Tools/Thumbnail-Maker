...
function filter(e) {
	...
	target.moving = true;
	e.clientX
		? ((target.oX = e.clientX), (target.oY = e.clientY))
		: ((target.oX = e.touches[0].clientX),
		  (target.oY = e.touches[0].clientY));
	...
	function drag(evt) {
		...
		e.clientX
			? ((target.lX = evt.clientX - target.oX),
			  (target.lY = evt.clientY - target.oY),
			  (target.oX = evt.clientX),
			  (target.oY = evt.clientY))
			: ((target.lX = evt.touches[0].clientX - target.oX),
			  (target.lY = evt.touches[0].clientY - target.oY),
			  (target.oX = evt.touches[0].clientX),
			  (target.oY = evt.touches[0].clientY));
...
function captionWriter() {
	...
	for (let i = 0; i < captions.length; i++) {
		...
		for (let i = 0; i < line.length; i++) {
			...
			ctxCaption.strokeText(
				line[i],
				captionPositionLeft,
				captionPositionTop + i * lineHeight
			);
			ctxCaption.fillText(
				line[i],
				captionPositionLeft,
...
function textAreaToDiv(e) {
	document.getElementsByClassName(
		`draggable  ${e.target.classList[1]}`
	)[0].textContent = e.target.value;
}
...
document.onkeydown = (e) => {
	if (darkText.indexOf(e.key.toLowerCase()) !== -1) {
		...
		keyCheat.join("") === "invert"
			? (darken(document.body.classList.contains("dark") ? "light" : "dark"),
			  (keyCheat = []))
			: null;
	}
};
...
function addOverlayImage() {
	...
	overlayCanvasDiv.addEventListener("click", removeOverlayAndDiv);
